##We’ve curated a set of interesting data sets for you. Pick one of those data sets or create a data set like that, and use advanced SELECT queries to 
discover things about the data. What sort of questions might one have about that data, like if they were using it for an app or a business idea?
...
##What are average, max, and min values in the data?
  SELECT ROUND(MAX(Winner_Pts),2) AS max_win,
      ROUND(MIN(Winner_Pts),2) AS min_win_pts,
      ROUND(AVG(Winner_Pts),2) AS avg_win_pts,
      ROUND(MAX(Loser_Pts),2) AS max_loss_pts,
      ROUND(MIN(Loser_Pts),2) AS min_loss_pts,
      ROUND(AVG(Loser_Pts),2) AS avg_loss_pts
  FROM superbowls;
...
##How many teams in the NFL have won the superbowl? 
SELECT COUNT(DISTINCT(Winner)) AS unique_wins, 
       COUNT(DISTINCT(Loser)) AS unique_losses
FROM superbowls;

##How many times has either the KC Chiefs or PA Eagles won a super bowl? 
SELECT COUNT(Winner) AS Champion
FROM superbowls
WHERE Winner = 'Kansas City Chiefs' OR Winner = 'Philadelphia Eagles';
